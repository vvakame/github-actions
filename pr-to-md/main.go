...
var (
	...
	owner        = kingpin.Flag("owner", "name of repository owner. default value is from GITHUB_EVENT_PATH.repository.owner.login .").String()
	name         = kingpin.Flag("name", "name of repository. default value is from GITHUB_EVENT_PATH.repository.name .").String()
...
func main() {
	...
	eventData := &githubv3.PullRequestEvent{}
	{
	...
	if *owner == "" {
		*owner = *eventData.Repo.Owner.Login
	}
	if *name == "" {
